...
def get_withdrawal_status(account, invoice_id=None):
    ...
    try:
        ...
        logging.info(f'get_withdrawal_status of invoice id [{invoice_id if invoice_id else invoice_id_value}]: {status_value} \n {res}')

    return res


def list_withdrawal_history(account, status="Processing", payment_type="withdrawal"):
    driver = webdriver.Firefox()
    res = {}
    res["data"] = {}
    ...
    try:
        ...
        xpath = '//*/div[@data-auto="filter-payment-status-option-done"]'
        xpath = '//*/div[@data-auto="filter-payment-status-option-processing"]'
        xpath = '//*/div[@data-auto="filter-payment-status-option-rejected"]'
        ...
        # 7. List rows of processing withdrawal
        logging.info(f'7. List rows of processing withdrawal')
        wait = WebDriverWait(driver, 10)
        xpath = f'//*/div[@data-auto="group-rows"]/*'       # list child tag <div> of parent
        wait.until(EC.element_to_be_clickable((By.XPATH, xpath)))
        rows = driver.find_elements(By.XPATH, xpath)
        logging.info(f'7. List rows of processing withdrawal: {len(rows)} rows')
        invoice_id_list = []
        for row in rows:
            ...
            logging.info(f'7. List rows of processing withdrawal: {attr_invoice_id}')
